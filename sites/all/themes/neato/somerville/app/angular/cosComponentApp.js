(function(){
    ...
    $locationProvider.hashPrefix('tab');
	}]);

	app.controller('TabCtrl', ['$scope', '$location', function ($scope, $location) {
		$scope.tabArg = $location.path();

		this.currentTab = 0;

		if ($scope.tabArg && $scope.tabArg !== '') {
			$scope.tabArg = $scope.tabArg.slice(1);
			$scope.tabInt = parseInt($scope.tabArg, 10);
			if ($scope.tabInt) {
				this.currentTab = $scope.tabInt;
			}
		}
